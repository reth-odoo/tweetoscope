...
function tweetParse(tweet_data: tweet_format, user_data: user_format, parent?: RawTweet): RawTweet{
    ...
    if(parent) {
        tweet = new RawTweet(tweet_data.id, user_data[author].name, user_data[author].username, user_data[author].profile_image_url, new Date(tweet_data.created_at), tweet_data.text, metrics, parent);
    }
    else{
        tweet = new RawTweet(tweet_data.id, user_data[author].name, user_data[author].username, user_data[author].profile_image_url, new Date(tweet_data.created_at), tweet_data.text, metrics);
    }
